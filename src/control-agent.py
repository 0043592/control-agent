...
try:
    ...
    sys.exit('Sorry. This script requires python3 >= 3.9 VERSION')
...
class NginxAgent:
    ...
    def setup(self, upstreams: typing.Union[None, typing.List], **kwargs):
        ...
        check_structure = [
            ...
            os.path.join(self.nginx_path, "upstreams"),
        ...
        # 3.3 create upstreams/backend.conf  /etc/nginx/upstreams/backend.conf
        upstreams_config = os.path.join(self.nginx_path, "upstreams", "backend.conf")
        self.create_config(
            path=upstreams_config,
            template_name="template.backend.conf",
            **{
                "upstreams": upstreams,
            })
    ...
    def run(store: Store, **kwargs):
        ...
        worker = Worker(store)
        # load config
        logger.info(f"control agent VERSION {get_version_full()}")
...
class Bootstrap:
    usage = """
    /usr/bin/control-agent <command> [<args>] -h 
    ...
    def command_run(parser: argparse.ArgumentParser = None):
        try:
            ...
            if control_agent_settings := os.environ.get('CONTROL_AGENT_SETTINGS'):
                control_agent_settings = json.loads(control_agent_settings)
                upstreams = control_agent_settings["upstreams"]
                connect_urls = control_agent_settings["control_urls"]
                connect_token = control_agent_settings["connect_token"]
            else:
